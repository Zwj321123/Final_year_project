...
'''
# initialize the predictor: load network model and config files
def initPredictor(configPath="./cfg/yolov4-flower.cfg", weightPath="./backup/yolov4-flower_best.weights", metaPath="./data/obj.data"):
    """
...
# convert "xywh" annotation format into "xyxy" annotation format
def xyxy2xywh(x):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right
    y0 = (x[0] + x[2]) / 2  # x center
    y1 = (x[1] + x[3]) / 2  # y center
    y2 = x[2] - x[0]  # width
    y3 = x[3] - x[1]  # height
    y = [y0, y1, y2, y3]
    return y
...
# Convert detection results into JSON format
def detections2json(detections):
    """
    Change the detections to json results for Flask
    """
    boxes_detected = []
    for i, val in enumerate(detections):
        box = xywh2xyxy(val[2])
        boxes_detected.append({"name": str(val[0], encoding="utf-8"),
                               "conf": str(val[1]),
...
if __name__ == "__main__":
    initPredictor()
    detections = performPredict(outPath="./frontend/static/output/img_predicted.jpg")
    detections2json(detections)
